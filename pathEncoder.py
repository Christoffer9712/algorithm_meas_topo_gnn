...
def _add_overlay_nodes_pyg(data, overlays, virtual_type="Virtual", device="cpu"):
    ...
    for i, overlay in enumerate(overlays):
        ...
        ovl_id, ovl = next(iter(overlay.items()))
        for node in ovl["path"]:
...
class GraphEncoder:
    ...
    def encode_overlays_pyg(self, data, overlays):
        ...
        for idx, overlay in enumerate(overlays):
            node_idx = graph.name_to_idx[f"OVL-{idx}"]
            ovl_id, _ = next(iter(overlay.items()))
            ovl_encodings[ovl_id] = encoding[node_idx]
        return ovl_encodings

    
def overlay_encodings(data, overlays, encoder, device="cpu"):
    graph_encoder = GraphEncoder(encoder, device=device)
    return graph_encoder.encode_overlays_pyg(data, overlays)
