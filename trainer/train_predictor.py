...
def train(dataset_path=None, model_dir=None, epochs=100, batch_size=16, lr=5e-4, device='cpu', min_delta=1e-4, patience=10):
    ...
    predictor = Predictor(in_dim=64 + 64 + 1, hidden_dim=128).to(device) # Input is H = [topo_embed || meas_embed || m]
    ...
    def run_t(t, gidx, dataset):
        ...
        if INCLUDE_MEAS:
            for i in range(0, Delta):
                ...
                h_hist = graph_encoder.encode_overlays_pyg(data, hist_overlay)
        ...
        for m in range(1, M+1):
            ...
            data = Data(x=d['x'], edge_index=d['edge_index'], edge_attr=d['edge_attr'], node_names=d['node_names'], name_to_idx=d['name_to_idx'])            
            hist_overlay = d['overlay_paths']
            h_topo = graph_encoder.encode_overlays_pyg(data, hist_overlay)
            for ovl in curr_overlay:
                ovl_id = ovl['id']
                if ovl_id in [path["id"] for path in d['overlay_paths']]:
                    if INCLUDE_MEAS:
                    ...
                    out = predictor(H)
                    tgt = torch.tensor(ovl['meas'], dtype=torch.float)
